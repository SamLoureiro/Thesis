...
- This model is used to classify bearings as healthy or damaged based on the reconstruction error of the autoencoder.
...
def main():
    ...
    features_df.to_csv('ae_detect_stft.csv', index=False)
    ...
    '''features = pd.read_csv('ae_detect_mfcc.csv')
    labels = pd.read_csv('ae_class_labels.csv') 
    ...
    good_bearing_labels = np.array([0] * good_bearing_samples.shape[0])
    
    damaged_bearing_samples = features[np.array(labels) == 2]
    damaged_bearing_labels = np.array([1] * damaged_bearing_samples.shape[0])   
    
    # Split noise data into training and validation sets
    X_train, X_val_complete, y_train, y_val_complete = train_test_split(good_bearing_samples, good_bearing_labels, test_size=0.2, random_state=42)
    ...
    damaged_val_complete, damaged_test_complete, damaged_val_labels_complete, damaged_test_labels_complete = train_test_split(damaged_bearing_samples, damaged_bearing_labels, test_size=0.5, random_state=42)
    
    print("\nNumber of good Training Samples:", len(X_train))
    print("Number of good Validation Samples:", len(X_val_complete))
    
    print("Number of damaged Validation Samples:", len(damaged_test_complete))
    print("Number of damaged Test Samples:", len(damaged_val_complete))   
    
    
    val_size = min(len(X_val_complete), len(damaged_val_complete))
    
    if(len(X_val_complete) > len(damaged_val_complete)):
        X_val = X_val_complete[:val_size]
        y_val = y_val_complete[:val_size]
    else:
        X_val = X_val_complete
        y_val = y_val_complete
        damaged_val = damaged_val_complete[:val_size]
        damaged_val_labels = damaged_val_labels_complete[:val_size]
    ...
    #test_size = min(len(damaged_test_labels_complete), len(noise_labels))
    
    test_size = min(len(damaged_test_labels_complete), len(X_test_complete))
    ...
    bearings_val = damaged_val[:val_size]
    bearings_test = damaged_test_complete[:test_size]
    bearings_val_labels = damaged_val_labels[:val_size]
    bearings_test_labels = damaged_test_labels_complete[:test_size]

    # The remaining bearing samples are used for the final evaluation
    only_bearings_eval = np.concatenate((damaged_val[val_size:], damaged_test_complete[test_size:]))
    ...
    output_dir_metrics = os.path.join(current_dir, 'AE_Results', 'NEW_AMR', 'IMPROVED_DATASET', 'Classification')
    model_save_path = os.path.join(current_dir, 'AE_Models', 'NEW_AMR', 'IMPROVED_DATASET','Classification', model_name)
    os.makedirs(output_dir_metrics, exist_ok=True)
        
    # For Bayesian tuning
    model_save_path_tuner = os.path.join(current_dir, 'AE_Models', 'NEW_AMR', 'IMPROVED_DATASET', 'Bayesian','Classification', model_name) 
    parameters_save_path = os.path.join(current_dir, 'AE_Models', 'NEW_AMR', 'IMPROVED_DATASET', 'Bayesian', 'Classification', save_parameters_name)
    output_dir_metrics_tuner = os.path.join(current_dir, 'AE_Models', 'NEW_AMR', 'Bayesian', 'IMPROVED_DATASET', 'Classification') 
    tuner_dir = os.path.join(current_dir, 'Bayesian_Tuning', 'NEW_AMR', 'AE_DENSE', 'IMPROVED_DATASET', 'Classification')    
    ...
    if(config.model_load and os.path.exists(model_save_path)):
        ...
        autoencoder = DENSE(
                    ...
                    bottleneck_units=64,                         # Custom number of bottleneck units
        ).build()
    ...
    probs_df = pd.DataFrame({
        ...
        'Predicted Probability': combined_test_errors
    })
    ...
    threshold_string = str(optimal_threshold.round(2))
    # Customize the plot
    plt.xlabel('Predicted Probability')
    plt.ylabel('Density')
    plt.title('Error Distribution for Healthy vs. Damaged Bearings')
