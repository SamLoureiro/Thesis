...
class DENSE:
    ...
    def build(self):
        ...
        # Decoder
        input_decoder = BatchNormalization()(bottleneck)
        decoded = Dropout(self.dropout_3)(input_decoder)
        decoded = Dense(self.units_2, activation='relu', kernel_regularizer=regularizers.L1L2(l1=self.l1, l2=self.l2))(bottleneck)
        decoded = BatchNormalization()(decoded)
        decoded = Dropout(self.dropout_4)(decoded)
        decoded = Dense(self.units_1, activation='relu', kernel_regularizer=regularizers.L1L2(l1=self.l1, l2=self.l2))(decoded)
        
        constructed = Dense(self.input_dim, activation='relu')(decoded)
        
        decoder = Model(inputs = input_decoder, outputs = constructed, name= 'decoder')
        decoder.summary()
        
        autoencoder = Model(input_layer, constructed)
