...
plot_dir = os.path.join(current_dir, 'Plots', 'ONLY_BEARING')
...
# Define feature extraction function for audio with optional preprocessing techniques
def extract_audio_features(file_path, noise_profile_file, apply_noise_reduction=True, apply_basics = True, apply_fft=True, apply_mfcc=True, apply_stft=True):
    # Load noise profile
    ...
    # Extract mean, std, rms, kurtosis, and skew
    if apply_basics:
        features['mean'] = np.mean(y)
        features['std'] = np.std(y)
        features['rms'] = np.sqrt(np.mean(y**2))
        features['kurtosis'] = kurtosis(y) if np.std(y) > epsilon else 0
        features['skew'] = skew(y) if np.std(y) > epsilon else 0

    # FFT between 500Hz and 96kHz
    if apply_fft:
        fft_result = np.abs(np.fft.fft(y, n=1024))
        freqs = np.fft.fftfreq(1024, 1/sr)
        fft_500_96k = fft_result[(freqs >= 500) & (freqs <= 85000)]
        fft_500_96k = fft_500_96k[:1024]  # Take the positive half of the FFT

        # Add FFT features
        for i, value in enumerate(fft_500_96k):
            features[f'fft_500_96k_{i}'] = value
    ...
    if apply_mfcc:
        mfccs = librosa.feature.mfcc(y=y, sr=sr, n_fft=1024, hop_length=512, 
                                     ...
                                     n_mels=20, fmin=1000, fmax=20000, n_mfcc=5)
    ...
    if apply_stft:
        ...
        for i, (mean, std) in enumerate(zip(stft_mean, stft_std)):
            features[f'stft_mean_{i}'] = mean
            features[f'stft_std_{i}'] = std
...
for audio_file in good_bearing_files_audio:
    audio_features = extract_audio_features(audio_file, noise_profile_file, apply_noise_reduction=False, apply_basics = False, apply_fft=True, apply_mfcc=False, apply_stft=False)
...
clf = tfdf.keras.GradientBoostedTreesModel(task=tfdf.keras.Task.CLASSIFICATION, num_trees=200, growing_strategy="BEST_FIRST_GLOBAL", max_depth=12)
...
plot_path = os.path.join(plot_dir, 'FFT_RF_ACC_LOSS.svg')
...
target_names = ['GOOD', 'SMALL_DAMAGED']
print(classification_report(y_test, y_pred, target_names=['SMALL_DAMAGED', 'GOOD']))
...
plot_path = os.path.join(plot_dir, 'FFT_RF_CM.svg')
plt.savefig(plot_path, format='svg')
